...
void CLOCK_Init()
{
    systick_set_clocksource(STK_CSR_CLKSOURCE_EXT); // M0 The RCC feeds the Cortex System Timer (SysTick) external clock with the AHB clock (HCLK) divided by 8
...
//    nvic_set_priority(NVIC_EXTI2_IRQ, 0x01); //Highest priority
    ...
    timer_callback = NULL;
    /* Enable TIMx clock. */
    rcc_periph_clock_enable(SYSCLK_TIM.rcc);

    /* Enable TIMx interrupt. */
    nvic_enable_irq(SYSCLK_TIM.irq);
    nvic_set_priority(SYSCLK_TIM.irq, 16);  // High priority

    timer_disable_counter(SYSCLK_TIM.tim);
    /* Reset TIMx peripheral. */
    rcc_periph_reset_pulse(SYSCLK_TIM.rst);
    ...
    nvic_set_priority(NVIC_EXTI0_1_IRQ, 64); //Medium priority
...
void CLOCK_RunMixer(void) {
    ...
    nvic_set_pending_irq(NVIC_EXTI0_1_IRQ);
}
