...
void __attribute__((__used__)) SYSCLK_TIMER_ISR()
{
    if(timer_callback) {
        ...
        if (us) {
            timer_set_oc_value(SYSCLK_TIM.tim, TIM_OC1, us + TIM_CCR1(SYSCLK_TIM.tim));
            return;
        }
    }
    CLOCK_StopTimer();
...
void __attribute__((__used__)) exti4_15_isr()
{
    // low_priority_cb();
    LED_Handle();    
}

void __attribute__((__used__)) exti0_1_isr()
{
...
//    }
    if(msec_callbacks & (1 << MEDIUM_PRIORITY)) {
        ...
        if (msecs == msec_cbtime[MEDIUM_PRIORITY]) {
            ...
            if (mixer_sync == MIX_TIMER) nvic_set_pending_irq(NVIC_EXTI0_1_IRQ);
    ...
    if(msec_callbacks & (1 << LOW_PRIORITY)) {
        if (msecs == msec_cbtime[LOW_PRIORITY]) {
            ...
            nvic_set_pending_irq(NVIC_EXTI4_15_IRQ);
