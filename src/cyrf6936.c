...
static void CS_HI(void) {
    PROTOSPI_pin_set(CYRF6936_Config.cs);
}

static void CS_LO(void) {
    PROTOSPI_pin_clear(CYRF6936_Config.cs);
}
...
int CYRF_Reset()
{
    PROTOSPI_pin_set(CYRF6936_Config.rst);
    Delay(100);
    PROTOSPI_pin_clear(CYRF6936_Config.rst);
    Delay(100);
    CYRF_WriteRegister(CYRF_0C_XTAL_CTRL, 0x00); // Enable 12Mhz XOUT
...
void CYRF_SetTxRxMode(enum TXRX_State mode)
{
    if(mode==TXRX_OFF) {
        ...
        PROTOSPI_pin_clear(CYRF6936_Config.tx);
        PROTOSPI_pin_clear(CYRF6936_Config.rx);
    } else {
    ...
    if(mode == TX_EN) {
        PROTOSPI_pin_clear(CYRF6936_Config.rx);
        PROTOSPI_pin_set(CYRF6936_Config.tx);
    } else if(mode == RX_EN) {
        PROTOSPI_pin_clear(CYRF6936_Config.tx);
        PROTOSPI_pin_set(CYRF6936_Config.rx);
    } else {
        PROTOSPI_pin_clear(CYRF6936_Config.tx);
        PROTOSPI_pin_clear(CYRF6936_Config.rx);
    }
