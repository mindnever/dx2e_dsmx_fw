...
//       u8 idx = chmap[upper * 7 + i];
       u8 idx = upper * 7 + i;
       s32 value;
       if ((idx == 0xff) || ((num_channels > 7) && (idx > num_channels - 1))) {
           value = 0xffff;
       } else {
...
static void cyrf_startup_config(void)
{
...
static void cyrf_transfer_config(void)
{
...
NO_INLINE static void parse_telemetry_packet(void)
{
